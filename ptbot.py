...
class Bot():
    ...
    def create_countdown(self, timeout_secs, callback, *args, **kwargs):
        ...
        if not timeout_secs:
            raise TypeError("Не могу запустить таймер на None секунд")
        if args:
            raise TypeError(f"create_countdown() takes 2 positional "
                            f"arguments but {len(args) + 2} were given")
        ...
        self.job_queue.run_repeating(wrapper, 1,
    ...
    def reply_on_message(self, callback, *args, **kwargs):
        if not callable(callback):
            raise TypeError('Ожидаем функцию на вход')
        if args:
            raise TypeError(f"reply_on_message() takes 1 positional "
                            f"argument but {len(args) + 1} were given")
